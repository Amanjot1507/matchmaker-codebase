...
public class FieldValueController{
	...
	public static void removeFieldValue(EntityManager em, FieldValue value) 
		...
		List<LatestAddition> results = (List<LatestAddition>) em.createQuery(query).setLockMode(LockModeType.PESSIMISTIC_READ).getResultList();
		try {
	...
	public static FieldValue getFieldValueById(EntityManager em, long id, String type) throws InstantiationException, IllegalAccessException{
        ...
        @SuppressWarnings("unchecked")
        List<FieldValue> itens = (List<FieldValue>) em.createQuery(query).setLockMode(LockModeType.PESSIMISTIC_READ).getResultList();
        try {
        	tx.commit();
        	return itens.get(0);
        }
	...
	public static String[] getArrayOfOfType(EntityManager em, String type) {
        ...
        try {
			@SuppressWarnings("unchecked")
			List<FieldValue> it = (List<FieldValue>) em.createQuery(query).setLockMode(LockModeType.PESSIMISTIC_READ).getResultList();
	...
	public static List<FieldValue> getListOfType(EntityManager em, String type) {
        ...
        try {
        	 @SuppressWarnings("unchecked")
        	 List<FieldValue> items = (List<FieldValue>) em.createQuery(query).setLockMode(LockModeType.PESSIMISTIC_READ).getResultList();
        	 tx.commit();
        	 return items;
        }
        catch (Exception e){
	...
	public static List<LatestAddition> getLatestAddedFields(EntityManager em) {
		...
		List<LatestAddition> items = (List<LatestAddition>) em.createQuery(query).setLockMode(LockModeType.PESSIMISTIC_READ).getResultList();
		tx.commit();
		return items;
	}
